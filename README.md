...
Iya, menggunakan konsep transformasi, transformasi rotasi dan transformasi translasi.

## 3. Jika iya maka jelaskan konsep transformasi apa yang ada pada gambar tersebut!
### a. Translasi
Translasi artinya objek dipindahkan dari koordinat lokal ke koordinat posisi sebenarnya di canvas. Bagian utamanya ada di fungsi:
```javascript
...
### b. Rotasi
Rotasi artinya objek diputar terhadap suatu sudut tertentu. Bagian utamanya:
```javascript
const SANG=-0.65;
...
Jadi satelit tidak digambar lurus horizontal/vertikal, tetapi diputar sebesar `-0.65` radian. Bagian yang terkena rotasi:
...
Ada juga konsep scaling pada tahap akhir, walaupun bukan scaling transform seperti `ctx.scale()`, tapi secara logika piksel diperbesar. Bagian ini:
```javascript
const S=3;
```
...
Tujuannya agar gambar terlihat seperti pixel art yang besar dan tajam. Jadi ada transformasi skala dalam bentuk pembesaran manual piksel.

---

### d. Rotasi pada nebula/ellipse
Bukan hanya satelit, latar nebula juga menggunakan rotasi. Bagian ini:
```javascript
...
Fungsi ini mengecek apakah suatu titik berada di dalam ellipse yang diputar. Dipakai pada bagian:
- pembuatan nebula
...
Ini memberi tepi nebula efek pixelated
...
#### a. Bintang biasa (bintang diletakkan di posisi acak)
Kode:
```javascript
for(let i=0;i<200;i++){
  ...
...
#### b. Tiny stars (titik kecil tambahan supaya langit lebih ramai)
Kode:
```javascript
for(let i=0;i<100;i++){
  ...
...
2. **Menggunakan konsep transformasi**  
   Iya, transformasi rotasi dan transformasi translasi.
